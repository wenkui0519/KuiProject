...
tinymce.PluginManager.add('emoji', function (editor, url) {
    ...
    var life = ['bamboo', 'gift_heart', 'dolls', 'school_satchel', 'mortar_board', 'flags',
        ...
        'pound', 'euro', 'credit_card', 'money_with_wings', 'e_mail', 'inbox_tray',
    ...
    var signs = ['one', 'two', 'three', 'four', 'five', 'six', 'seven',
        ...
        'signal_strength', 'u5272', 'u5408', 'u55b6', 'u6307', 'u6708', 'u6712',
        ...
        'underage', 'no_mobile_phones', 'do_not_litter', 'non_potable_water',
    ...
    var recent = localStorage.getItem("emoji_recent") ? localStorage.getItem("emoji_recent").split(".") : [];
    var all = [recent, faces, nature, life, travel, signs],
        allName = ["recent", "faces", "nature", "life", "travel", "signs"],
        showRecent = recent.length > 0 ? 0 : 1;

    function getHtml() {
        var emojiHtml = "";
        if (recent.length > 0) {
            emojiHtml = "<div class='emoji-container emoji-content'>\
                            <div class='emoji-groups'>\
                                <i class='emoji-group icon-recent icon-recent-selected' data-icon='recent' id='icon_recent' title='最近使用'></i>\
                                <i class='emoji-group icon-smile' data-icon='smile' id='icon_faces'></i>\
                                <i class='emoji-group icon-flower' data-icon='flower' id='icon_nature'></i>\
                                <i class='emoji-group icon-bell' data-icon='bell' id='icon_life'></i>\
                                <i class='emoji-group icon-car' data-icon='car' id='icon_travel'></i>\
                                <i class='emoji-group icon-grid' data-icon='grid' id='icon_signs'></i>\
                            </div><div class='emoji-table'>";
        } else {
            emojiHtml = "<div class='emoji-container emoji-content'>\
                            <div class='emoji-groups'>\
                                <i class='emoji-group icon-recent' data-icon='recent' id='icon_recent' title='最近使用'></i>\
                                <i class='emoji-group icon-smile icon-smile-selected' data-icon='smile' id='icon_faces'></i>\
                                <i class='emoji-group icon-flower' data-icon='flower' id='icon_nature'></i>\
                                <i class='emoji-group icon-bell' data-icon='bell' id='icon_life'></i>\
                                <i class='emoji-group icon-car' data-icon='car' id='icon_travel'></i>\
                                <i class='emoji-group icon-grid' data-icon='grid' id='icon_signs'></i>\
    ...
    function getCategoryEmoji() {
        ...
        for (var i = 0; i < 6; i++) {
            ...
            categoryHtml += i == showRecent ? "<table class='emoji-list mce-grid current' id='" + allName[i] + "'><tr>" :
                "<table class='emoji-list mce-grid' id='" + allName[i] + "'><tr>";
            for (var j = 0; j < length; j++) {
                if (j % 7 == 0 && j != 0) {
                    categoryHtml += "</tr><tr>";
                }
                var className = all[i][j].replace(reg, "-");
                categoryHtml += "<td><i class='emoji-picker emoji-" + className + "' data-mce-url = 'emoji-" + className + "'></i></td>";
            }
    ...
    function createPopover(event) {
        var popover = document.getElementById('editor_popover_20210412'),
            bg = document.getElementById('editor_bg_20210412');
        if (popover) {
            ...
            bg.setAttribute('id', 'editor_bg_20210412');
            document.body.appendChild(bg);
            jQuery(bg).css({ 'position': 'fixed', 'left': '0', 'top': '0', 'right': '0', 'bottom': '0', 'z-index': '99' })

            jQuery('#editor_bg_20210412').click(() => {
                bg.style.display = "none";
                popover.style.display = "none";
            })
            jQuery(popover).click((e) => {
                if (e) {
                    e.stopPropagation();
                }
                console.log(1)
                var linkElm = editor.dom.getParent(e.target, 'i');
                if (e.target.tagName === 'I' && e.target.className.includes('emoji-picker')) {
                    const str = '<i class="emoji-picker mceNonEditable ' + linkElm.getAttribute('data-mce-url') + '">&nbsp;[' + linkElm.getAttribute('data-mce-url') + ']</i>'
                    editor.insertContent(str);
                    recent = localStorage.getItem("emoji_recent") ? localStorage.getItem("emoji_recent").split(".") : [];
                    var reg = new RegExp("_", "g"),
                        emoji_name = linkElm.getAttribute('data-mce-url').replace("emoji-", "").replace(reg, "_");
                    if (recent.indexOf(emoji_name) === -1) {
                        if (recent.length > 68) {
                            recent.pop();
                        }
                        recent.unshift(emoji_name);
                    } else {
                        recent.splice(recent.indexOf(emoji_name), 1);
                        recent.unshift(emoji_name);
                    }
                    var html = "";
                    for (var i = 0; i < recent.length; i++) {
                        ...
                        var className = recent[i].replace(reg, "-");
                        html += "<td><i class='emoji-picker emoji-" + className + "' data-mce-url = 'emoji-" + className + "'></i></td>";
                    }
                    html += "</tr>";
                    document.getElementById("recent").innerHTML = html;
                    recent = recent.join(".");
                    localStorage.setItem("emoji_recent", recent);
                    bg.style.display = "none";
                    popover.style.display = "none";
                }
                if (e.target.tagName === 'I' && e.target.className.includes('emoji-group')) {
                    var id = linkElm.id,
                        icon = linkElm.getAttribute("data-icon"),
                        tableId = id.replace("icon_", ""),
                        emojiGroup = document.getElementsByClassName("emoji-group"),
                        emojiTable = document.getElementsByClassName("emoji-list");
                    for (var i = 0; i < emojiGroup.length; i++) {
                        emojiGroup[i].className = emojiGroup[i].className
                            .replace("icon-" + emojiGroup[i].getAttribute("data-icon") + "-selected", "");
                    }
                    for (var i = 0; i < emojiTable.length; i++) {
                        emojiTable[i].className = emojiTable[i].className.replace("current", "");
                    }
                    document.getElementById(tableId).className += " current";
                    linkElm.className = linkElm.className + " icon-" + icon + "-selected";
                }
            })
        }
    }
